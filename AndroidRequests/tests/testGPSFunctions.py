...
class TestGPSFunctions(TestCase):
	...
	def test_getGPSData(self):
		self.timeStamp = datetime.strptime("2016-11-29 12:46:20","%Y-%m-%d %H:%M:%S")
		plon = -70.63098
		plat = -33.46516
		jsonContent = self.jsonData
		longitud, latitud, time, distance = Gps.getGPSData("FLXC49", self.timeStamp, plon, plat, jsonContent)
		...
		self.assertEqual(time, parse_datetime(data['nearestGpsPoint']['time'] + Constants.TIMEZONE))
		self.assertGreaterEqual(distance, expectedDistance - expectedDistance*self.error)
		self.assertLessEqual(distance, expectedDistance + expectedDistance*self.error)
