...
class ServiceRoute(View):
    ...
    def get(self, request, pBusService, pLat1, pLon1, pLat2, pLon2):
        ...
        if len(route) == 0:
            ...
            response['statusMessage'] = "Service does not have route in the database."
    ...
    def getRoutesForService(self, pServiceCode):
        ...
        for service in Route.objects.values('serviceCode').filter(serviceCode__regex=service).distinct():
            variantCode = service['serviceCode']
            variant = {}
            variant['variant'] = variantCode
            variant['route'] = []
            for point in Route.objects.filter(serviceCode=variantCode).order_by('sequence'):
