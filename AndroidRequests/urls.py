...
urlpatterns = [
    ...
    url(
        ...
        RegisterEventBus.as_view()),
    url(
        r'^reportEventBusStop/(?P<pUserId>[0-9a-z-]+)/(?P<stopCode>[\w,0-9]*)/(?P<pEventID>evn\d{5})/(?P<pLatitud>[\-+]?[0-9]*\.?[0-9]*)/(?P<pLongitud>[\-+]?[0-9]*\.?[0-9]*)/(?P<pConfirmDecline>(confirm|decline))$',
        RegisterEventBusStop.as_view()),
    url(
        r'^reportEventBusStop/(?P<pUserId>[0-9a-z-]+)/(?P<stopCode>[\w,0-9]*)/(?P<pEventID>evn\d{5})/(?P<pConfirmDecline>(confirm|decline))$',
        RegisterEventBusStop.as_view()),
    url(
        r'^reportEventBusStop/(?P<pUserId>[0-9a-z-]+)/(?P<stopCode>[\w,0-9]*)/(?P<pService>[0-9,\w]*)/(?P<pEventID>evn\d{5})/(?P<pLatitud>[\-+]?[0-9]*\.?[0-9]*)/(?P<pLongitud>[\-+]?[0-9]*\.?[0-9]*)/(?P<pConfirmDecline>(confirm|decline))$',
        RegisterEventBusStop.as_view()),
    url(
        r'^reportEventBusStop/(?P<pUserId>[0-9a-z-]+)/(?P<stopCode>[\w,0-9]*)/(?P<pService>[0-9,\w]*)/(?P<pEventID>evn\d{5})/(?P<pConfirmDecline>(confirm|decline))$',
        RegisterEventBusStop.as_view()),
